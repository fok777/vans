    ...
    private void invokeInitInternal(String packageName, ModuleLoader loader) {
        ...
        dataMap.forEach((className, data) -> {
            ...
            try {
                ...
                // 原实现只捕获 ReflectiveOperationException。低版本系统上还可能抛
                // NoClassDefFoundError / NoSuchMethodError / LinkageError 等 Error，
                // 一旦逃出 lambda 就会中断 dataMap 剩余规则的加载。这里按 Throwable 兜底。
